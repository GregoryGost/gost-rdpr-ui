...
/**
 * Domains Lists management page
 * @component DomainsListsPage
 */
...
const selectedList = ref<DomainsList | null>(null)
 ...
 */
const TABLE_COLUMNS: TableColumn<DomainsList>[] = [
  ...
  {
    key: 'attempts',
    label: DOMAINS_LISTS_TEXTS.COLUMN_ATTEMPTS,
    sortable: true,
    filterValue: (_row: DomainsList, value: unknown) => {
...
function matchesListColumnFilters(list: DomainsList): boolean {
  return matchesColumnFilters(list, TABLE_COLUMNS, columnFilters.value)
}

/**
 * Search domains lists
 */
const showSearchResults = (total: number) => {
  ...
  showInfo(`${DOMAINS_LISTS_TEXTS.SEARCH_FOUND_PREFIX} ${total}`, UI_TEXTS.SEARCH_RESULTS)
}
...
const loadSearchResults = async (shouldNotify = false) => {
  ...
  try {
    const response = await loadAttemptsFilteredPage(
      (params) => domainsListsApi.search(searchQuery.value, params),
      {
...
const validateForm = (): boolean => {
  ...
  if (!formData.value.name || formData.value.name.length < VALIDATION.MIN_NAME_LENGTH) {
    formErrors.value.name = DOMAINS_LISTS_TEXTS.VALIDATION_NAME_REQUIRED
    showWarning(DOMAINS_LISTS_TEXTS.VALIDATION_NAME_REQUIRED, ERROR_MESSAGES.VALIDATION_ERROR)
    return false
  }

  if (!formData.value.url || formData.value.url.length < VALIDATION.MIN_URL_LENGTH) {
    formErrors.value.url = DOMAINS_LISTS_TEXTS.VALIDATION_URL_REQUIRED
    showWarning(DOMAINS_LISTS_TEXTS.VALIDATION_URL_REQUIRED, ERROR_MESSAGES.VALIDATION_ERROR)
    return false
  }
  ...
  try {
    ...
    formErrors.value.url = DOMAINS_LISTS_TEXTS.VALIDATION_URL_INVALID
    showWarning(DOMAINS_LISTS_TEXTS.VALIDATION_URL_INVALID, ERROR_MESSAGES.VALIDATION_ERROR)
    return false
  }
 ...
 * Open view modal
 */
const openViewModal = (list: DomainsList) => {
...
const deleteList = async () => {
  ...
  isLoading.value = true
  try {
    await domainsListsApi.deleteOne(listId)
...
<template>
  <div>
    <!-- Header -->
    <div class="mb-6">
      <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">{{ DOMAINS_LISTS_TEXTS.PAGE_TITLE }}</h1>
      <p class="mt-2 text-gray-600 dark:text-gray-400">
        {{ DOMAINS_LISTS_TEXTS.PAGE_DESCRIPTION }}
      </p>
    </div>
    ...
    <div class="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-4">
      <div class="rounded-lg border bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
        ...
        <p class="mt-2 text-xs text-gray-500 dark:text-gray-500">{{ DOMAINS_LISTS_TEXTS.STATS_TOTAL_HINT }}</p>
      </div>
      <div class="rounded-lg border bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
        ...
        <p class="mt-2 text-xs text-gray-500 dark:text-gray-500">{{ DOMAINS_LISTS_TEXTS.STATS_SUCCESS_HINT }}</p>
      </div>
      <div class="rounded-lg border bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
        ...
        <p class="mt-2 text-xs text-gray-500 dark:text-gray-500">{{ DOMAINS_LISTS_TEXTS.STATS_ERRORS_HINT }}</p>
      </div>
      <div class="rounded-lg border bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
        ...
        <p class="mt-2 text-xs text-gray-500 dark:text-gray-500">{{ DOMAINS_LISTS_TEXTS.STATS_CRITICAL_HINT }}</p>
      </div>
    </div>
    ...
    <div class="mb-6 flex flex-col justify-between gap-4 lg:flex-row">
      ...
      <div class="flex flex-col gap-4 sm:flex-row sm:items-center">
        ...
        <div class="relative max-w-md flex-1">
          ...
          <input
            ...
            @input="searchLists"
            type="search"
            :placeholder="DOMAINS_LISTS_TEXTS.SEARCH_PLACEHOLDER"
        ...
        <div class="flex flex-wrap gap-2">
          <BaseButton
            ...
            size="sm"
          >
            {{ DOMAINS_LISTS_TEXTS.FILTER_ALL }}
          </BaseButton>
          <BaseButton
            ...
            size="sm"
          >
            {{ DOMAINS_LISTS_TEXTS.FILTER_SUCCESS }}
          </BaseButton>
          <BaseButton
            ...
            size="sm"
          >
            {{ DOMAINS_LISTS_TEXTS.FILTER_ERRORS }}
          </BaseButton>
          <BaseButton
      ...
      <BaseButton @click="openAddModal" variant="primary">
        ...
        {{ DOMAINS_LISTS_TEXTS.ADD_BUTTON }}
      </BaseButton>
    </div>
    ...
    <DataTable
      ...
      :empty-message="DOMAINS_LISTS_TEXTS.EMPTY_MESSAGE"
    ...
    <!-- Add Modal -->
    <BaseModal :is-open="isAddModalOpen" :title="DOMAINS_LISTS_TEXTS.MODAL_TITLE" @close="closeAddModal">
      <form @submit.prevent="createList" class="space-y-4">
        <div>
          <label class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">
            {{ DOMAINS_LISTS_TEXTS.LABEL_NAME }} <span class="text-red-500">*</span>
          </label>
          <BaseInput
            v-model="formData.name"
            type="text"
            :placeholder="DOMAINS_LISTS_TEXTS.PLACEHOLDER_NAME"
        ...
        <div>
          <label class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">
            {{ DOMAINS_LISTS_TEXTS.LABEL_URL }} <span class="text-red-500">*</span>
          </label>
          <BaseInput
            v-model="formData.url"
            type="url"
            :placeholder="DOMAINS_LISTS_TEXTS.PLACEHOLDER_URL"
            :error="formErrors.url"
            required
          />
          <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
            {{ DOMAINS_LISTS_TEXTS.HINT_URL }}
          </p>
        </div>

        <div>
          <label class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">
            {{ DOMAINS_LISTS_TEXTS.LABEL_DESCRIPTION }}
          </label>
          <BaseTextarea
            v-model="formData.description!"
            :placeholder="DOMAINS_LISTS_TEXTS.PLACEHOLDER_DESCRIPTION"
            :rows="2"
          />
        </div>
    ...
    <BaseModal
      ...
      :title="`${DOMAINS_LISTS_TEXTS.LIST_PREFIX}: ${selectedList?.name || ''}`"
        ...
        <div class="space-y-3">
          ...
          <div>
            <label class="mb-1 block text-xs font-medium text-gray-500 dark:text-gray-400">
              {{ DOMAINS_LISTS_TEXTS.LABEL_NAME }}
            </label>
          ...
          <div>
            <label class="mb-1 block text-xs font-medium text-gray-500 dark:text-gray-400">
              {{ DOMAINS_LISTS_TEXTS.COLUMN_ATTEMPTS }}
            </label>
          ...
          <div>
            <label class="mb-1 block text-xs font-medium text-gray-500 dark:text-gray-400">
              {{ DOMAINS_LISTS_TEXTS.COLUMN_DOMAINS_COUNT }}
            </label>
            <p class="text-sm text-gray-900 dark:text-gray-100">
              <span class="font-mono font-semibold">{{ selectedList.elements_count }}</span> доменов
            </p>
          </div>
    ...
    <ConfirmDialog
      ...
      :title="DOMAINS_LISTS_TEXTS.DELETE_TITLE"
      :message="DOMAINS_LISTS_TEXTS.DELETE_MESSAGE"
