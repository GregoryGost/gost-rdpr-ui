...
Откладывает выполнение задачи до завершения серии вызовов и автоматически отменяет
ожидающий запуск при размонтировании компонента. Задержка по умолчанию —
`SEARCH.DEBOUNCE_MS` (500 мс).
...
Эти константы обеспечивают единообразие во всем приложении и упрощают глобальное
изменение настроек.
...
1. ErrorHandler анализирует тип ошибки и определяет severity
1. Создается уведомление с соответствующим сообщением
1. Уведомление отображается в правом верхнем углу экрана
1. Автоматически закрывается по истечении таймера
...
- [Статус проекта](../../docs/PROJECT_STATUS.md) - детальный статус разработки, история изменений
