...
1. 🏠 Главная → `/`
1. 🖥️ DNS Серверы → `/dns`
1. 📄 Списки Доменов → `/domains/lists`
1. @ Домены → `/domains`
1. 📋 Списки IP Адресов → `/ips/lists` ⭐ NEW
1. 📍 IP Адреса → `/ips`
1. ⚙️ Конфигурации RoS → `/ros`
1. 💻 Команды → `/commands`
...
1. ✅ DNS Servers Page - Управление DNS серверами
1. ✅ Domains Lists Page - Управление списками доменов
1. ✅ Domains Page - Управление доменами
1. ✅ IPs Lists Page - Управление списками IP адресов
1. ✅ IPs Page - Управление IP адресами
1. ✅ ROS Configs Page - Управление конфигурациями RouterOS
1. ✅ Commands Page - Выполнение команд
...
1. 📄 Списки Доменов (зелёный)
1. @ Домены (фиолетовый)
1. 📋 Списки IP Адресов (оранжевый)
1. 📍 IP Адреса (розовый)
1. ⚙️ Конфигурации RoS (индиго)
1. 💻 Команды (красный)
...
1. ✅ DNS Серверы - таблица, пагинация
1. ✅ Списки Доменов - статистика, фильтры, таблица
1. ✅ Домены - множественные фильтры
1. ✅ Списки IP Адресов - структура подтверждена
1. ✅ IP Адреса - большие таблицы (87К записей)
1. ✅ Конфигурации RouterOS - форма с паролями
1. ✅ Команды - управление командами
...
Глобальные счётчики на страницах `DomainsPage`, `DomainsListsPage`, `IpsPage` и
`IpsListsPage` переведены с накопления из пагинации на `statsApi.getStats()`:
...
SVG с `viewBox="0 0 800 H"` и `width: 100%` масштабирует все атрибуты (`font-size`,
`stroke-width`, `r`) пропорционально контейнеру. На 1920px шрифт 11px SVG-единиц
рендерится в ~23px — непропорционально крупно.
...
- Размеры полос вычисляются как plain-числа внутри `barItems` computed — устранена ошибка
  Vue `BAR_HEIGHT not defined on instance` при использовании `ComputedRef` в арифметике шаблона
...
- `/domains/lists`, `/domains/lists/search`, `/ips/lists`, `/ips/lists/search` отправляют
  `attempts=0` для фильтра `Без ошибок`
...
## 📝 Обновления (2026-05-14 — 2026-05-16)

### ✅ Пагинация и фильтры таблиц

- Фильтры по диапазонам `attempts` применяются ко всему набору до пагинации.
- Фильтры колонок на серверно-пагинируемых страницах переведены в controlled-режим с согласованными `payload` и `total`.
- Для условий без полноценной backend-фильтрации добавлены `loadClientFilteredPage` и `loadAttemptsFilteredPage`.
- Поиск и фильтры колонок используют общий `useDebouncedTask`; текущая задержка — 500 мс.

### ✅ Версия приложения

- Версия из `package.json` отображается в footer через `__APP_VERSION__`.
- Добавлены GitHub Actions для тегирования merge в ветки `develop` и `main`.

---

## 📝 Обновления (2026-07-17)

### ✅ Проверка резолвинга доменов

- Добавлена разовая проверка `POST /domains/resolve/check` по доменному имени или ID без сохранения результата.
- Результат показывает ответы каждого DNS-сервера: IPv4, IPv6 и CNAME.
- Доступен ручной выбор одного или нескольких DNS-серверов; без `dns_server_ids`
  используются пользовательские серверы, а ID `0` служит fallback.
- При явном ручном выборе DNS ID `0` используется как обычный выбранный сервер.

### ✅ Фоновый резолвинг сохранённого домена

- В действия строки добавлен отдельный запуск `POST /domains/{id}/resolve` для положительных ID.
- Модальное окно сообщает о принятии фоновой операции (`202`), не выдавая его за завершённый резолвинг.
- Версия приложения обновлена до `1.1.0`.

---

## 📝 Обновления (2026-07-29)

### ✅ RIPEstat для IP-адресов

- Добавлена проверка IPv4-префикса через `POST /ips/ripe/check`; она не изменяет записи в
  БД и доступна в строке сохранённого IPv4 с положительным ID.
- Добавлена общая операция `POST /ips/ripe/cache/clear`, расположенная рядом с добавлением IP-адреса.
- Очистка удаляет только memory-кеш RIPEstat и не изменяет БД; перед выполнением интерфейс запрашивает подтверждение.

### ✅ Tailwind CSS и настройки редактора

- Глобальный стиль перенесён в `src/css/main.css`: он подключает Tailwind, определяет
  токены, базовые слои и пользовательский dark-вариант.
- Dark-вариант использует `@custom-variant` и класс `dark` на корневом HTML-элементе.
- Устранены рекомендации Tailwind IntelliSense для устаревших и неканонических utility-классов.
- Для Vue-файлов в `.vscode/settings.json` Prettier выбран форматировщиком рабочей области.
